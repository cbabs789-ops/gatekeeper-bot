...
class Runner:
    def __init__(self):
        self.con = db.connect()
        self.p = config.strategy_params()
        self.safety = {r["mint"]: dict(r) for r in self.con.execute("SELECT * FROM safety")}
        self.strat = Strategy(self.p, lambda m: self.safety.get(m))
        self.session = None
        self._restore()
    ...
    def _restore(self):
        ...
        saved_max = self.strat.p["MAX_OPEN"]
        self.strat.p["MAX_OPEN"] = 0                 # rebuild peaks and history without trading
        for r in rows:
            c = coins.get(r["mint"])
            if c:
                self.strat.on_snapshot(dict(r), c)
        self.strat.p["MAX_OPEN"] = saved_max
        for r in self.con.execute("SELECT mint FROM trades WHERE mode='live'"):
            self.strat.traded.add(r["mint"])
        for r in self.con.execute("SELECT * FROM trades WHERE mode='live' AND closed_at IS NULL"):
            ...
            for leg in legs[1:]:
                if leg["side"] == "sell":
                    ...
                    pos.took_half = True
            cs = self.strat.coins.get(r["mint"])
            pos.peak_after = cs.peak_price if cs else pos.spot_at_entry
            self.strat.positions[r["mint"]] = pos
        log.info("Restored %d coins, %d open positions", len(self.strat.coins), len(self.strat.positions))
    ...
    async def poll_once(self):
        ...
        for i in range(0, len(mints), 30):
            ...
            for mint, pair in pairs.items():
                ...
                for a in self.strat.on_snapshot(s, c):
                    await self.act(a)
                age_min = (now - c["graduated_at"]) / MIN
                if s["liq"] < p["DEAD_LIQ_USD"] and age_min > 30 and mint not in self.strat.positions:
        ...
        for mint, c in coins.items():
            age_min = (now - c["graduated_at"]) / MIN
            if mint in self.strat.positions:
                continue
            if age_min > p["WATCH_HOURS"] * 60:
                ...
                self.con.execute("UPDATE coins SET status='dead' WHERE mint=?", (mint,))
        for a in self.strat.on_tick(now):
            await self.act(a)
        ...
        while True:
            try:
                ...
                for r in rows:
                    ...
                    if age < self.p["MIN_AGE_MIN"] - 10 or (r["liq"] or 0) < self.p["MIN_LIQ_USD"] * 0.7:
                        continue
    ...
    async def act(self, a):
        ...
        if a["type"] == "buy":
            cur = self.con.execute(
                "INSERT INTO trades(mode, mint, symbol, opened_at, entry_price, size_usd, qty, why_entered, legs) "
                "VALUES('live',?,?,?,?,?,?,?,?)",
                (pos.mint, pos.symbol, pos.opened_at, pos.entry_price, pos.size_usd, pos.qty_total, pos.why, json.dumps(pos.legs)))
            pos.trade_id = cur.lastrowid
            await notify.send(self.session, notify.fmt_buy(pos, a["spot"], a["liq"], self.p))
        elif a["type"] == "partial":
            self.con.execute("UPDATE trades SET legs=? WHERE id=?", (json.dumps(pos.legs), pos.trade_id))
            await notify.send(self.session, notify.fmt_partial(pos, a["spot"], a["usd"]))
        elif a["type"] == "close":
            ...
            await notify.send(self.session, notify.fmt_close(pos, a["spot"], a["reason"]))
            ...
            try:
                for u in await notify.get_updates(self.session, offset):
                    ...
                    if cmd in ("/status", "status"):
                        await notify.send(self.session, report.status_text(self.con, self.strat))
                        ...
                        await notify.send(self.session, "Commands: /status (open trades and feed health), /today, /week")
    ...
    async def daily_loop(self):
        while True:
            ...
            if now.hour == 21 and db.kv_get(self.con, "summary_sent") != key:
                await notify.send(self.session, "📊 <b>Daily paper summary</b>\n" + report.period_text(self.con, hours=24, header=False))
    ...
    async def run(self):
        async with aiohttp.ClientSession(headers={"User-Agent": "gatekeeper-bot/1.0"}) as session:
            ...
            await notify.send(session, "🤖 Gatekeeper bot started. Watching %d coins, %d open paper trades. Send /status anytime."
                              % (len(self.strat.coins), len(self.strat.positions)))
